  ...
  visitor: ts.Visitor
    ...
    let props = ts.createObjectLiteral(
      attributes.map((attr) => {
        ...
        (attr as any).initializer
      )}
    ))

    // 如果有children 的话
    if (nodeChilds.length) {
      var children = ts.createArrayLiteral(nodeChilds.map(v => visitor(v)))
    }

    return ts.createCall(ts.createIdentifier('h'), [], [ts.createLiteral(openingElement.tagName.getText()), props, children])
}